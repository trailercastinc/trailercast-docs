...
- Browse modal opens are **always free** (FTS + cached AI reads).
- Each explicit "Ask AI to rank" or "Refresh" click costs ~$0.05, logged in your cost analysis under `library_ai_suggest`.
- Repeat clicks on the same Space's modal are free (cache hits) until the rep manually refreshes.

Typical power-user daily cost: $0.05–$0.15 — only paid for Spaces where the rep actually wants AI help.
