...
The **Conversations inbox** — the speech-bubble icon in the top ribbon (it moved out of the left sidebar in August 2026) — shows every active prospect thread across every demo you've sent. An unread badge on the icon tells you when a prospect has replied. Useful when you want to see "what's open with anyone" without clicking into each demo separately.

### Who the thread is about

Each row identifies the **client**, not just the sender:

- A **company chip** next to the sender's name. Threads attached to a Decision Room show that room's company; other threads infer the company from the prospect's email domain. Personal addresses (gmail and friends) show no chip — a mail provider isn't a client.
- The sender's **name and email shown separately**, instead of one `Name <email>` string.
- An **Internal** badge on threads started from your own company's domain — that's you or a teammate testing a share link, not a prospect.

*Rolling out from the week of 2026-08-31 — if you don't see company chips yet, the update hasn't reached your workspace.*
...
- **By prospect or client**, matches names, emails, and company chips
- **By date range**, custom from/to dates

### Sort + actions

- Newest activity first, always
- **Open conversation** takes you to where the thread lives — the Decision Room's Conversations section for deal-attached threads, the share page otherwise
- **Open in trailers** jumps to the demo the thread is about
- The trash icon deletes an entire thread (both sides of it) permanently
...
- **For multi-stakeholder deals, use a Buyer Space instead.** The Conversations inbox is for single-prospect threads; Spaces handle teams. See [Buyer Spaces](/buyer-spaces).
